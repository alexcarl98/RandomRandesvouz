...
The scatterplot Displays the two arrival times, `time1` for Kareem and `time2` for Kareem and Kathleen. 
For each of the 10,000 repetitions. The green points represent the pairs of arrival times where the two people successfully meet, while the red points represent the pairs of arrival times where the two people do not meet. 
The empirical probability that Kareem and Kathleen meet is about 0.4342.
...
$$
> This value is very close to what we got in the above section, so it is in the error bound.
...
Through algebraic manipulation, we will see that in order to get 17.57359313 min for a 50% chance

for a 90% change, it will be about 41.0 minutes
...
### **Comment**
This distribution is different since it no longer takes up the entirety of the square. Instead, the values are concentrated in an ellipse shape near the mean of 30 minutes.
